...
function App() {
  ...
  useEffect(() => {
    const newFilteredData = data.filter(datum => datum.timestamp >= min && datum.timestamp <= max);
  ...
  return (
    <Container>
      ...
      <p>
        A dataset of ~900K ride share trips in the city of Austin, TX from May 2016 to Feb 2017.
      </p>
      ...
      <Row>
        <Col sm={12} lg={6}>
          ...
          <Row>
            <Col><p align={"left"}>
              Min: {formatTime(new Date(displayMin))}
            </p></Col>
            <Col><p align={"right"}>
              Max: {formatTime(new Date(displayMax))}
            </p></Col>
          </Row>
          <Range
            min={extent[0]}
